 ...
 *           which it writes to the console.
 *
 * @attention
 *
 ...
 * NOTES:
 *
 * 1. The single-sided two-way ranging scheme implemented here has to be 
 *    considered carefully as the accuracy of the distance measured is highly
 *    sensitive to the clock offset error between the devices and the length 
 *    of the response delay between frames. To achieve the best possible
 *    accuracy, this response delay must be kept as low as possible. In order 
 *    to do so, 6.8 Mbps data rate is used in this example and the response
 *    delay between frames is defined as low as possible. The user is referred 
 *    to User Manual for more details about the single-sided two-way ranging
 *    process.  NB:SEE ALSO NOTE 11.
 * 2. The sum of the values is the TX to RX antenna delay, this should be 
 *    experimentally determined by a calibration process. Here we use a hard 
 *    coded value (expected to be a little low so a positive error will be 
 *    seen on the resultant distance estimate. For a real production application, 
 *    each device should have its own antenna delay properly calibrated to get 
 *    good precision when performing range measurements.
 * 3. The frames used here are Decawave specific ranging frames, complying with 
 *    the IEEE 802.15.4 standard data frame encoding. The frames are the
 *    following:
 *     - a poll message sent by the initiator to trigger the ranging exchange.
 *     - a response message sent by the responder to complete the exchange and
 *       provide all information needed by the initiator to compute the
 ...
 *     - byte 9: function code (specific values to indicate which message 
 *               it is in the ranging process).
 ...
 * 4. Source and destination addresses are hard coded constants in this 
 *    example to keep it simple but for a real product every device should 
 *    have a unique ID. Here, 16-bit addressing is used to keep the messages 
 *    as short as possible but, in an actual application, this should be done 
 *    only after an exchange of specific messages used to define those short 
 *    addresses for each device participating to the ranging exchange.
 * 5. This timeout is for complete reception of a frame, i.e. timeout 
 *    duration must take into account the length of the expected frame. 
 *    Here the value is arbitrary but chosen large enough to make sure that 
 *    there is enough time to receive the complete response frame sent by 
 *    the responder at the 6.8M data rate used (around 200 usec).
 ...
 * 7. dwt_writetxdata() takes the full size of the message as a parameter but
 ...
 *    bytes.
 * 9. The high order byte of each 40-bit time-stamps is discarded here. 
 ...
 *    which means that the calculation of the round-trip delays can be handled
 *    by a 32-bit subtraction.
 ...
 * 11. The use of the carrier integrator value to correct the TOF calculation,
 *     was added Feb 2017 for v1.3 of this example.  This significantly
 *     improves the result of the SS-TWR where the remote responder unit's 
 *     clock is a number of PPM offset from the local inmitiator unit's clock.
 *     As stated in NOTE 2 a fixed offset in range will be seen unless the 
 *     antenna delsy is calibratred and set correctly.
 *
 ******************************************************************************/
